...
namespace MyHttpServer.Handler
{
    public class StaticFilesHandler: IHandler
    {
        ...
        private HtmlTemplateReader _htmlReader;

        public StaticFilesHandler(ServerConfiguration configuration)
        {
            _configuration = configuration;
            _htmlReader = new HtmlTemplateReader();
        }
        ...
        private async Task HandleStatic(HttpListenerContext context)
        {
            try
            {
                ...
                if (Path.GetExtension(filePath) == ".html")
                    await _htmlReader.UpdateHtmlPage(context, filePath);
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Error handling request: {ex.Source} {ex.Message}");
                Console.ResetColor();
            }
        }
