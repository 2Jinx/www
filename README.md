...
1) Написать игру "Точка". Должен быть клиент и сервер.  
Правила игры: первая форма - это вход в игру, где необходимо передать `username`.
После входа в игру справа должен быть расположен список всех подключенных игроков (их никнеймы с разными цветами). Слева поле, где при нажатии кнопки мыши рисуется точка вашего цвета и передается другим игрокам. И у них также происходит рисование данной точки. При отключении игрока список должен обновляться. При подключении нового игрока, точки которые были уже на холсте должны  у него подгрузиться.

#### PixelBattle

- [Проект](https://github.com/2Jinx/www/tree/main/homework_11/ "Проект")
